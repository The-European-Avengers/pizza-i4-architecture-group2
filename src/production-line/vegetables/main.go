...
func main() {
    ...
    writerFreezer := &kafka.Writer{
        Addr:     kafka.TCP(kafkaAddr),
        Topic:    produceTopicFreezer,
        Balancer: &kafka.LeastBytes{},
    }
    defer writerFreezer.Close()

    fmt.Printf("🎧 Listening on '%s' and producing to  '%s' and '%s'...\n", consumeTopic, produceTopicOven, produceTopicFreezer)
    ...
    for {
        ...
        case <-ctx.Done():
            ...
            return
        case msg := <-msgChan:
            processAndProduce(ctx, writerOven, writerFreezer, msg)
        }
    }
}
...
func processAndProduce(ctx context.Context, writerOven *kafka.Writer, writerFreezer *kafka.Writer, order Order) {
    fmt.Printf("🥦 Preparing vegetables for order %d...\n", order.Id)
    time.Sleep(time.Duration(rand.Intn(1000)+500) * time.Millisecond)
    fmt.Printf("🥦 Vegetables added to pizza for order %d.\n", order.Id)
    ...
    // Decide where to produce based on some condition
    if order.Baked {
        writer = writerFreezer
    } else {
        writer = writerOven
    }
